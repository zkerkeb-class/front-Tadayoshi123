...
interface DashboardGridProps {
  ...
  isEditing?: boolean;
  onLayoutChange?: (layout: any, blocks: DashboardBlock[]) => void;
...
export function DashboardGrid({
  ...
  const handleLayoutChange = (currentLayout: any, allLayouts: any) => {
    ...
    const updatedBlocks = blocks.map(block => {
      const layoutItem = currentLayout.find((item: any) => item.i === block.id);
      if (layoutItem) {
        return {
          ...block,
          layout: {
    ...
    onLayoutChange(allLayouts, updatedBlocks);
  };
